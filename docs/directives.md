...
Extract the value of a claim to a new header value (base64url encoded JSON value), when proxing request. One location may extract multiple claims.
...
> [!NOTE]
> The module still returns `500` on internal error. This response code is only for validation failures.
